(function($) {
    ...
    $(function () {
        ...
        //Top Navigation
        $('.nav-item-container li', secondaryNavbar).each(function() {
            var $this = $(this);
            if ($this.find('a').length) {
                $($this.find('a')).each(function(index, element){
                    if ($(this).attr("href").indexOf(current) !== -1) {
                        $(this).closest('li').parents(':eq(2)').addClass('active');
                        $(this).closest('li').parents(':eq(2)').parents(':eq(2)').addClass('active');
